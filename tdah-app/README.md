...
- **Deux palettes complètes** (`lightColors`/`darkColors` dans `constants/theme.ts`) — le mode sombre n'est pas un simple assombrissement automatique : fond anthracite légèrement violet (jamais noir pur, pour limiter l'éblouissement en faible luminosité), couleurs vives éclaircies individuellement pour rester lisibles sur fond sombre.
...
Décidée en regardant le vrai écran de paiement de Tiimo (notre concurrent direct le plus établi aux USA), capture d'écran à l'appui — pas une hypothèse tirée d'un article marketing : sur cet écran, seule "Planification priorisée" a une coche en colonne Gratuit ; liste de tâches, co-planificateur IA, minuteur de concentration, activités en direct/widgets, application web et intégration calendrier sont tous exclusivement Pro. Consigne explicite du produit : coller à ce modèle, en encore plus strict s'il le faut — l'objectif assumé est de pousser vers l'abonnement, pas de maximiser le temps passé en gratuit.

- **Répartition retenue** — gratuit et illimité : planning quotidien (priorité, angoisse, moment de journée, ajout/édition/report de tâche), vues liste/frise, streak/badges/récompenses, check-ins, rappels locaux, Coach IA proactif (zéro coût, agrégation locale, rattaché à la planification de base), **mode sombre et mode focus** (seule exception non négociable : verrouiller de l'accessibilité sur une app pensée pour le TDAH contredirait la mission, quel que soit l'objectif de conversion). Tout le reste est premium : Vide-tête, Assistant conversationnel, découpage IA d'une tâche, Analyse de patterns long terme (tout ce qui est IA, façon "Co-planificateur IA"), **Backlog et Routines** (façon "Liste de tâches"), **le minuteur de concentration dans son ensemble** — solo (co-régulation/responsabilisation) et à deux — façon "Minuteur de concentration", personnalisation icône/couleur, synchronisation calendrier, suivi du sommeil, Bilan hebdomadaire réflexif.
- **Mise en place** (`lib/billing/stripe.ts`) : `usePremiumGate()` expose `gate(label, action)` — exécute l'action si l'abonnement (ou l'essai) est actif, sinon affiche une alerte au ton cohérent avec le reste de l'app (jamais culpabilisante) puis propose `/paiement`. `<PremiumScreen label="...">` (`components/PremiumScreen.tsx`) verrouille un écran entier (Backlog, Routines, Focus, Sommeil, Bilan hebdomadaire) en filet de sécurité si quelqu'un y accède directement par lien profond plutôt que par un bouton déjà verrouillé — pour Focus, tout l'écran (y compris le minuteur solo) est derrière ce filet, pas seulement Focus à deux.
- **Infrastructure de quota IA dormante** (`ai_usage`, migration `0016_ai_usage_limits.sql`) : un premier essai de cette stratégie laissait 3 usages IA gratuits/mois plutôt qu'un blocage total, façon "Tiimo garde son IA limitée mais gratuite" — abandonné une fois la vraie capture d'écran de paiement Tiimo examinée (elle ne montre aucune coche gratuite sur le co-planificateur IA). La table et les fonctions SQL (`record_ai_usage`, `get_ai_usage`) restent en base, côté client plus rien ne les appelle — gardées au cas où une v2 de la stratégie y revienne.
...
- **Présentation plus douce du verrouillage** : les icônes 🔒 (cadenas, connotation négative/blocage) ont été remplacées par des puces "✨ Premium" et des icônes étincelle dans le menu Outils, le Vide-tête et le découpage IA — même information, cadrage aspirationnel plutôt que punitif. Le texte de `gate()`/`<PremiumScreen>` ne prétend plus que "le focus" est gratuit, puisque ce n'est plus le cas.
- **Limite connue** : la vérification est côté client uniquement pour l'instant — les fonctions SQL des fonctionnalités premium ne vérifient pas encore l'abonnement ni l'essai elles-mêmes. Suffisant pour une V1 sans utilisateurs adverses, mais une vraie protection des revenus demandera d'ajouter ce contrôle aussi côté serveur (voir "Pas encore fait" ci-dessous).
