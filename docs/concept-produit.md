...
La fonctionnalité "détection de pattern doomscrolling" (vitesse de scroll dans une autre app) est **abandonnée**, y compris comme fonctionnalité Android-only envisagée un temps : la contrainte de fond est plus large qu'un simple problème de vitesse — **l'app ne s'exécute pas du tout pendant qu'une autre app est au premier plan**, sur iOS comme en pratique sur Android dès qu'on sort du cadre d'AccessibilityService. Aucun capteur (y compris les capteurs de mouvement, cf. ci-dessous) ne peut donc rien mesurer "pendant" une session TikTok. Toute fonctionnalité doit agir **avant ou après** cette session, sur les seuls signaux qu'Apple/Android exposent réellement (le temps cumulé). Deux fonctionnalités remplacent la détection de vitesse, toutes deux cross-platform et fondées sur ce levier temporel :

- **Récompense décroissante** : au lieu d'un budget de session fixe (10 min à chaque fois), les paliers diminuent au fil des réouvertures dans la journée (10 min → 5 min → 2 min → 2 min...). Ça retourne le mécanisme d'incertitude du renforcement variable contre lui-même — au lieu d'un contenu imprévisible, c'est la durée d'accès qui devient imprévisiblement de plus en plus courte. Voir `ios-app/App/Monitoring/SessionBudgetPlan.swift`.
- **Le "témoin" (accountability partner)** : quand l'utilisateur force l'accès, l'app propose d'envoyer un message pré-rempli à un contact de son choix via le partage natif (pas d'envoi automatique caché, Apple l'interdit — l'utilisateur doit appuyer lui-même sur "envoyer"). Aucun concurrent (Opal, Freedom, One Sec) ne propose cet angle social. Voir `ios-app/App/Accountability/AccountabilityPrompt.swift`.
