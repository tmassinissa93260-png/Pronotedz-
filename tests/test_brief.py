...
def test_un_brief_vide_est_refuse():
    ...
    with pytest.raises(ErreurValidation) as e:
        BriefWriter().apres({"sujet_nettoye": "un sujet", "beats": []},
...
def test_trop_peu_de_beats_est_refuse():
    ...
    with pytest.raises(ErreurValidation) as e:
        BriefWriter().apres({"sujet_nettoye": "un sujet", "beats": trop_court},
                            {"univers": u, "duree_s": 45}, _contexte())
...
def test_des_positions_dans_le_desordre_sont_refusees():
    ...
    with pytest.raises(ErreurValidation) as e:
        BriefWriter().apres({"sujet_nettoye": "un sujet", "beats": beats},
                            {"univers": u, "duree_s": 45}, _contexte())
...
def test_un_brief_valide_passe():
    ...
    sortie = BriefWriter().apres({"sujet_nettoye": "un sujet", "beats": beats},
                                 {"univers": u, "duree_s": 45}, _contexte())
    assert len(sortie["beats"]) == attendus


# ── Sujet nettoyé : séparer le sujet réel d'une remarque adressée à l'IA ──

def test_un_sujet_nettoye_vide_est_refuse():
    u = Univers.charger(FRUITS)
    attendus = nb_beats_pour(45)
    beats = [_beat(pct) for pct in range(0, attendus * 10, 10)][:attendus]
    with pytest.raises(ErreurValidation) as e:
        BriefWriter().apres({"sujet_nettoye": "   ", "beats": beats},
                            {"univers": u, "duree_s": 45}, _contexte())
    assert "sujet_nettoye" in str(e.value)


def test_un_sujet_nettoye_absent_est_refuse():
    u = Univers.charger(FRUITS)
    attendus = nb_beats_pour(45)
    beats = [_beat(pct) for pct in range(0, attendus * 10, 10)][:attendus]
    with pytest.raises(ErreurValidation) as e:
...
def test_le_schema_impose_le_sujet_nettoye():
    schema = charger("ecriture/brief").schema_sortie
    assert "sujet_nettoye" in schema["required"]
    assert schema["properties"]["sujet_nettoye"]["type"] == "string"
