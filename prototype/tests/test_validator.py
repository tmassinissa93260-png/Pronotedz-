...
class TestSeparation(unittest.TestCase):
    """L'image dit ce qui EXISTE, l'animation dit ce qui CHANGE."""

    def test_une_animation_qui_redit_l_image(self):
        raw = board()
        # L'animation recopie la description au lieu de decrire le changement.
        raw["shots"][0]["animation_prompt"] = (
            "Macro shot of the battery pack beneath the floor, prismatic cells running "
            "left to right at centre frame, copper busbars above them leading forward "
            "toward the electric motor, its stator windings and the central rotor. "
            "Controlled yellow luminous energy streams travel along the busbars. "
            "Camera at low angle, shallow depth. Everything remains rigid. "
            "As they move, the rotor begins to rotate and by the end turns steadily.")
        p = [x for x in valider(raw) if x.code == "SEPARATION"]
        self.assertTrue(p)
        self.assertIn("redit l'image", str(p[0]))

    def test_une_animation_sans_preservation(self):
        """Trois animations sur quatre du run 22 ne disaient pas ce qui reste fixe."""
        raw = board()
        raw["shots"][0]["animation_prompt"] = (
            "Yellow energy pulses begin at the cells and travel along the busbars "
            "toward the stator, which makes the central rotor accelerate from rest. "
            "By the end it turns at a steady speed, and the camera performs a slow "
            "lateral tracking movement following the energy path.")
        p = [x for x in valider(raw) if x.code == "SEPARATION"]
        self.assertTrue(p)
        self.assertIn("rester fixe", str(p[0]))

    def test_un_etat_de_depart_n_est_pas_une_preservation(self):
        """« the rotor is initially stationary » dit d'ou l'on part, pas ce
        qui reste fixe. Le plan 4 du run 22 passait grace a ce mot."""
        raw = board()
        raw["shots"][0]["animation_prompt"] = (
            "Initially, the rotor is stationary. Yellow energy pulses travel along the "
            "busbars toward the stator, which makes the rotor accelerate. By the end it "
            "turns steadily, and the camera performs a slow lateral tracking movement.")
        p = [x for x in valider(raw) if x.code == "SEPARATION"]
        self.assertTrue(p)
        self.assertIn("rester fixe", str(p[0]))

    def test_une_animation_bien_separee_passe(self):
